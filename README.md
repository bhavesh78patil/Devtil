...
| **SSH / PuTTY** | Open **multiple SSH sessions** (password auth) as terminal panels in one tab. Run commands per session, or type once in the **broadcast bar** to run the same command across every session with 📡 enabled (MobaXterm-style multi-exec). Each session panel minimizes, maximizes and closes independently; sessions and their output persist |
