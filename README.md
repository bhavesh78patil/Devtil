...
| **Oracle** | SQL console using a pure-Go driver — **no Oracle client install required**. Connect with separate host/port/service fields **or paste a JDBC/Easy-Connect URL** (`jdbc:oracle:thin:@host:1521/service`, `@//host:port/service`, or `host:port:SID`). Results grid, DML support (rows-affected), and the same **query helper**: pick from `user_tables`, choose columns, and the `SELECT … FETCH FIRST n ROWS ONLY` is generated |
